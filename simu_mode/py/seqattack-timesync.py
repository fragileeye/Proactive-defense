    ...
    def statistic(self):
        ...
        for i in range(self.N):
            ...
            if collision > collision_size: 
                collision += 1
        return collision / self.N


class SeqAttack_TimeSync:
...
def main():
    ...
    sim = SeqAttack_TimeSync(N, M, s, e, t1, t0)
...
def auto_test():
    ...
    #0.01s->1packet, 60s->6000packets in 80Mbps
    t1 = 100

    N_list = [500*i for i in range(1, 11)]
    e_list = [100000*i for i in range(1,11)]

    with open('seqattack-timesync.txt', 'wb+') as fp:
        for N in N_list:
            for e in e_list:
                #for t1 in t1_list:
                sim = SeqAttack_TimeSync(N, M, s, e, t1, t0)
