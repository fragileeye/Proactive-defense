	...
	)
{
	...
	//phase 1: hopping base on linkage fields
	text = ip_h->id ^ (tcp_h->seq ^ tcp_h->ack);

	hmac_md5(hopping_key, 64, (unsigned char *)&text, 4,  hmac);
	...
	//phase 2: mutating linkage fields
	text = ip_h->saddr ^ ip_h->daddr;

	hmac_md5(hopping_key, 64, (unsigned char *)&text, 4, hmac);

	tcp_h->seq ^= *(unsigned int *)hmac;
	tcp_h->ack ^= *(unsigned int *)&hmac[4];
	ip_h->id ^= *(unsigned short *)&hmac[8];
}

static void dehopping_packet(
	struct iphdr *ip_h, 
	struct tcphdr *tcp_h
	)
{
